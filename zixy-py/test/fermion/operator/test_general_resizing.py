...
def test_string_growth_and_no_shrinking():
    ...
    for length in (1, 2, 65):
        ...
        assert len(caught) == 1
        assert string.max_len == length
        assert string.get_ops() == [(0, False)] * length
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        string.set("F1^")
        string.clear()
    assert string.max_len == 65
...
def test_parsed_initial_capacity_is_silent(cls):
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        assert cls.from_str("F1 F0^", 2, max_len=1).max_len == 2
        assert cls.from_str("F1 F0^", 2, max_len=80).max_len == 80
        assert cls.from_str("", 2, max_len=80).max_len == 80
...
def test_sequence_preallocation_and_streaming(cls):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        value = cls.from_iterable(source, 2, max_len=1)
    assert value.max_len == 2
    consumed = []
...
def test_term_growth(term_type, array_type, set_type, sum_type, coeff):
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        term = term_type(2, ("F0 F1", coeff), max_len=1)
    assert term.string.max_len == 2
    with pytest.warns(UserWarning) as caught:
