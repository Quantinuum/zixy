...
pub fn mat_elem_cmpnts<C: FieldElem>(
    ...
    let bra_word = bra.get_u64it().next().unwrap();
    let ket_word = ket.get_u64it().next().unwrap();
    let mut contrib_sum = C::ZERO;
    for (op_cmpnt, op_coeff) in op.word_iters.iter().zip(op.coeffs.iter()) {
        let cre = op_cmpnt.get_cre_part().get_u64it().next().unwrap();
        let ann = op_cmpnt.get_ann_part().get_u64it().next().unwrap();
...
mod tests {
    ...
    use crate::fermion::state::terms::AsViewMut as _;
